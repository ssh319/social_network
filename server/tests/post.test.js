...
afterEach(async () => {
    ...
    await User.findByIdAndUpdate(exampleUserId, { $set: { posts: [] } });
});
...
describe("Post API endpoints", () => {
    ...
    describe("likePost", () => {
        test("should push user's id to 'likes' array of the post", async () => {
            const response = await request(app)
                .post(`${BASE_URL}/${examplePostId}/likes`)
                .set('Authorization', `Bearer ${friendToken}`);
        ...
        test("should respond with 'invalid id' error", async () => {
            await checkInvalidId('post', `${BASE_URL}/111/likes`, friendToken);
        });

        test("should respond with 'no such post' error", async () => {
            await checkNotFound('post', `${BASE_URL}/${notExistingId}/likes`, friendToken);
        });
    ...
    describe("unlikePost", () => {
        test("should pull the user's id from 'likes' array of the post", async () => {
            ...
            const response = await request(app)
                .delete(`${BASE_URL}/${examplePostId}/likes`)
                .set('Authorization', `Bearer ${friendToken}`);
        ...
        test("should respond with 'invalid id' error", async () => {
            await checkInvalidId('delete', `${BASE_URL}/111/likes`, friendToken);
        });

        test("should respond with 'no such post' error", async () => {
            await checkNotFound('delete', `${BASE_URL}/${notExistingId}/likes`, friendToken);
        });
    ...
    describe("sendPostComment", () => {
        test("should push the new comment object to post's 'comments' array", async () => {
            const response = await request(app)
                .post(`${BASE_URL}/${examplePostId}/comments`)
                .send({
                    text: "Comment test"
                })
                .set('Authorization', `Bearer ${userToken}`);
        ...
        test("should respond with 'invalid id' error", async () => {
            await checkInvalidId('post', `${BASE_URL}/111/comments`, userToken);
        });

        test("should respond with 'no such post' error", async () => {
            // !
            const response = await request(app)
                .post(`${BASE_URL}/${notExistingId}/comments`)
                .send({
    ...
    describe("deletePostComment", () => {
        test("should delete the provided comment from post document", async () => {
            const response = await request(app)
                .delete(`${BASE_URL}/${examplePostId}/comments/${exampleCommentId}`)
                .set('Authorization', `Bearer ${userToken}`);
        ...
        test("should respond with 'invalid id' error", async () => {
            await checkInvalidId('delete', `${BASE_URL}/111/comments/${notExistingId}`, userToken);
            await checkInvalidId('delete', `${BASE_URL}/${examplePostId}/comments/111`, userToken);
        });

        test("should respond with 'not found' errors", async () => {
            await checkNotFound('delete', `${BASE_URL}/${notExistingId}/comments/${notExistingId}`, userToken);
            await checkNotFound('delete', `${BASE_URL}/${examplePostId}/comments/${notExistingId}`, userToken);
        });
    ...
    describe("likePostComment", () => {
        test("should add user's id to a post's 'likes' array", async () => {
            const response = await request(app)
                .post(`${BASE_URL}/${examplePostId}/comments/${exampleCommentId}/likes`)
                .set('Authorization', `Bearer ${friendToken}`);
        ...
        test("should respond with 'invalid id' error", async () => {
            await checkInvalidId('post', `${BASE_URL}/111/comments/${notExistingId}/likes`, friendToken);
            await checkInvalidId('post', `${BASE_URL}/${examplePostId}/comments/111/likes`, friendToken);
        });

        test("should respond with 'not found' errors", async () => {
            await checkNotFound('post', `${BASE_URL}/${notExistingId}/comments/${notExistingId}/likes`, friendToken);
            await checkNotFound('post', `${BASE_URL}/${examplePostId}/comments/${notExistingId}/likes`, friendToken);
        });
    ...
    describe("unlikePostComment", () => {
        test("should remove user's id from post's 'likes' array", async () => {
            ...
            const response = await request(app)
                .delete(`${BASE_URL}/${examplePostId}/comments/${exampleCommentId}/likes`)
                .set('Authorization', `Bearer ${friendToken}`);
        ...
        test("should respond with 'invalid id' error", async () => {
            await checkInvalidId('delete', `${BASE_URL}/111/comments/${notExistingId}/likes`, friendToken);
            await checkInvalidId('delete', `${BASE_URL}/${examplePostId}/comments/111/likes`, friendToken);
        });

        test("should respond with 'not found' errors", async () => {
            await checkNotFound('delete', `${BASE_URL}/${notExistingId}/comments/${notExistingId}/likes`, friendToken);
            await checkNotFound('delete', `${BASE_URL}/${examplePostId}/comments/${notExistingId}/likes`, friendToken);
        });
